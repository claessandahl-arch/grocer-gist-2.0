...
BEGIN
    ...
    WITH exploded_items_with_index AS (
        SELECT 
            ...
            -- Safe casting: filters will run, but to be extra safe we could use exception handling, 
            -- but mirroring the View's WHERE clause is the first step.
        ...
        WHERE 
            ...
            AND (item->>'price') ~ '^\d+(\.\d+)?$' -- Regex check for number format if possible, or just trust the next line
