-- Grocery Wrapped: Fix aggregate function with set-returning function error
-- Refactor to use CTEs instead of nested set-returning functions in aggregates

DROP FUNCTION IF EXISTS get_wrapped_overview(INTEGER);
DROP FUNCTION IF EXISTS get_wrapped_products(INTEGER);
DROP FUNCTION IF EXISTS get_wrapped_patterns(INTEGER);

-- Function 1: Get Wrapped Overview
CREATE OR REPLACE FUNCTION get_wrapped_overview(year_param INTEGER)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'unique_products', (SELECT COUNT(*) FROM product_list),
...
CREATE OR REPLACE FUNCTION get_wrapped_products(year_param INTEGER)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, auth
...
CREATE OR REPLACE FUNCTION get_wrapped_patterns(year_param INTEGER)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, auth
    ...
    'store_loyalty_percent', (
      ...
      FROM (
        SELECT
          ...
          (SELECT COUNT(*) FROM base_receipts) AS total_visits
      ) loyalty
    ),
